 ...
 * <p>
 * BaseDateTimeField is thread-safe and immutable, and its subclasses must be as
 * well.
 *
...
public abstract class BaseDateTimeField extends DateTimeField {
	...
	// -----------------------------------------------------------------------
	/**
	 * Get the human-readable, text value of this field from the milliseconds. If
	 * the specified locale is null, the default locale is used.
	 * <p>
	 * The default implementation returns getAsText(get(instant), locale).
	 *
	 * @param instant the milliseconds from 1970-01-01T00:00:00Z to query
	 * @param locale  the locale to use for selecting a text symbol, null means
	 *                default
	...
	/**
	 * Get the human-readable, text value of this field from a partial instant. If
	 * the specified locale is null, the default locale is used.
	 * <p>
	 ...
	 * {@link ReadablePartial#get(DateTimeFieldType)} and
	 * {@link #getAsText(ReadablePartial, int, Locale)}.
	 *
	 * @param partial the partial instant to query
	 * @param locale  the locale to use for selecting a text symbol, null for
	 *                default
	...
	/**
	 * Get the human-readable, text value of this field from the field value. If the
	 * specified locale is null, the default locale is used.
	 * <p>
	...
	// -----------------------------------------------------------------------
	/**
	 * Adds a value (which may be negative) to the instant value, overflowing into
	 * larger fields if necessary.
	 * <p>
	 * The value will be added to this field. If the value is too large to be added
	 * solely to this field, larger fields will increase as required. Smaller fields
	 * should be unaffected, except where the result would be an invalid value for a
	 * smaller field. In this case the smaller field is adjusted to be in range.
	 * <p>
	 ...
	 * necessary. Smaller fields should be unaffected, except where the result would
	 * be an invalid value for a smaller field. In this case the smaller field is
	...
	public int[] add(ReadablePartial instant, int fieldIndex, int[] values, int valueToAdd) {
		...
		// there are more efficient algorithms than this (especially for time only
		// fields)
	 ...
	 * <p>
	 * Partial instants only contain some fields. This may result in a maximum
	 * possible value, such as TimeOfDay normally being limited to 23:59:59:999. If
	 * this limit is reached by the addition, this method will wrap back to
	 * 00:00:00.000. In fact, you would generally only use this method for classes
	 * that have a limitation such as this.
	 * <p>
	 * For example, in the ISO chronology:<br>
	...
	/**
	 * Adds a value (which may be negative) to the instant value, wrapping within
	 * this field.
	 * <p>
	 * The value will be added to this field. If the value is too large to be added
	 * solely to this field then it wraps. Larger fields are always unaffected.
	 * Smaller fields should be unaffected, except where the result would be an
	 * invalid value for a smaller field. In this case the smaller field is adjusted
	 * to be in range.
	 * <p>
	 * For example, in the ISO chronology:<br>
	 ...
	 * <p>
	 * The default implementation internally calls set. Subclasses are encouraged to
	...
	/**
	 * Adds a value (which may be negative) to the partial instant, wrapping within
	 * this field.
	 * <p>
	 * The value will be added to this field. If the value is too large to be added
	 * solely to this field then it wraps. Larger fields are always unaffected.
	...
	public int[] addWrapField(ReadablePartial instant, int fieldIndex, int[] values, int valueToAdd) {
		...
		int wrapped = FieldUtils.getWrappedValue(current, valueToAdd, getMinimumValue(instant),
	 ...
	 * <p>
	 * The value of this field will be set. If the value is invalid, an exception if
	 * thrown.
	 * <p>
	 * If setting this field would make other fields invalid, then those fields may
	 * be changed. For example if the current date is the 31st January, and the
	 * month is set to February, the day would be invalid. Instead, the day would be
	 * changed to the closest value - the 28th/29th February as appropriate.
	 * 
	 ...
	 * <p>
	 * The value of this field (specified by the index) will be set. If the value is
	 * invalid, an exception if thrown.
	 * <p>
	 * If setting this field would make other fields invalid, then those fields may
	 * be changed. For example if the current date is the 31st January, and the
	 * month is set to February, the day would be invalid. Instead, the day would be
	...
	public int[] set(ReadablePartial partial, int fieldIndex, int[] values, int newValue) {
		FieldUtils.verifyValueBounds(this, newValue, getMinimumValue(partial, values),
	...
	// ------------------------------------------------------------------------
	/**
	 * Returns the duration per unit value of this field. For example, if this field
	 * represents "hour of day", then the unit duration is an hour.
	 *
	 * @return the duration of this field, or UnsupportedDurationField if field has
	 *         no duration
	 */
	 ...
	 * specified values.
	 * <p>
	 * This implementation returns the same as
	 * {@link #getMinimumValue(ReadablePartial)}.
	 * 
	 * @param instant the partial instant to query
	 ...
	 * <p>
	 * This implementation returns the same as
	 * {@link #getMaximumValue(ReadablePartial)}.
	 * 
	 * @param instant the partial instant to query
	 * @param values  the values to use
	 * @return the maximum value for this field, in the units of the field
	...
	/**
	 * Get the maximum text value for this field. The default implementation returns
	 * the equivalent of Integer.toString(getMaximumValue()).length().
	 * 
	...
	/**
	 * Get the maximum short text value for this field. The default implementation
	 * returns getMaximumTextLength().
	 * 
	 * @param locale the locale to use for selecting a text symbol
	...
	// ------------------------------------------------------------------------
	/**
	 * Round to the lowest whole unit of this field. After rounding, the value of
	 * this field and all fields of a higher magnitude are retained. The fractional
	 * millis that cannot be expressed in whole increments of this field are set to
	 * minimum.
	 * <p>
	...
	/**
	 * Round to the highest whole unit of this field. The value of this field and
	 * all fields of a higher magnitude may be incremented in order to achieve this
	 * result. The fractional millis that cannot be expressed in whole increments of
	 * this field are set to minimum.
	 * <p>
	 * For example, a datetime of 2002-11-02T23:34:56.789, rounded to the highest
	 * whole hour is 2002-11-03T00:00:00.000.
	 * <p>
	 * The default implementation calls roundFloor, and if the instant is modified
	 * as a result, adds one field unit. Subclasses are encouraged to provide a more
	...
	/**
	 * Round to the nearest whole unit of this field. If the given millisecond value
	 * is closer to the floor or is exactly halfway, this function behaves like
	 * roundFloor. If the millisecond value is closer to the ceiling, this function
	 * behaves like roundCeiling.
	 *
	 * @param instant the milliseconds from 1970-01-01T00:00:00Z to round
	 * @return rounded milliseconds
	 */
	...
	/**
	 * Round to the nearest whole unit of this field. If the given millisecond value
	 * is closer to the floor, this function behaves like roundFloor. If the
	 * millisecond value is closer to the ceiling or is exactly halfway, this
	 ...
	 * Round to the nearest whole unit of this field. If the given millisecond value
	 * is closer to the floor, this function behaves like roundFloor. If the
	 * millisecond value is closer to the ceiling, this function behaves like
	 * roundCeiling.
	 * <p>
	 * If the millisecond value is exactly halfway between the floor and ceiling,
	 * the ceiling is chosen over the floor only if it makes this field's value
	 * even.
	 *
	 * @param instant the milliseconds from 1970-01-01T00:00:00Z to round
	 * @return rounded milliseconds
	...
	/**
	 * Returns the fractional duration milliseconds of this field. In other words,
	 * calling remainder returns the duration that roundFloor would subtract.
	 * <p>
	 * For example, on a datetime of 2002-11-02T23:34:56.789, the remainder by hour
	 * is 34 minutes and 56.789 seconds.
	 * <p>
